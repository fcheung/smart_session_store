...
class LegacySmartSessionTest < ActiveSupport::TestCase
  ...
  def assert_final_session expected
    consolidated_session = LegacySmartSessionStore.new(@cgi_session)
    assert_equal expected, consolidated_session.restore
  end
  
  def setup_base_session
    base_session = LegacySmartSessionStore.new(@cgi_session)
    yield base_session if block_given?
    base_session.close
  end
  
  def do_simultaneous_session_access
    first_session = LegacySmartSessionStore.new(@cgi_session)
    second_session = LegacySmartSessionStore.new(@cgi_session)
